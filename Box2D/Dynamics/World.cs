...
#warning "PTR"
        ...
        public Fixture RaycastOne(Segment segment, out Fix64 lambda, out Vec2 normal, bool solidShapes, object userData)
        {
            int maxCount = 1;
            Fixture[] fixture;
            lambda = Fix64.Zero;
        ...
        private void SolveTOI(TimeStep step)
        {
            ...
            for (; ; )
            {
                ...
                if (minContact == null || Fix64.One - 100.Fix64.Zero * Settings.FLT_EPSILON < minTOI)
                {
        ...
        private void DrawJoint(Joint joint)
        {
            ...
            Color color = new Color(Fix64.Half, 0.8f, 0.8f);
            ...
            if ((flags & DebugDraw.DrawFlags.Shape) != 0)
            {
                ...
                for (Body b = _bodyList; b != null; b = b.GetNext())
                {
                    ...
                    for (Fixture f = b.GetFixtureList(); f != null; f = f.Next)
                    {
                        if (b.IsStatic())
                        {
                            DrawFixture(f, xf, new Color(Fix64.Half, 0.9f, Fix64.Half), core);
                        }
                        else if (b.IsSleeping())
                        {
                            DrawFixture(f, xf, new Color(Fix64.Half, Fix64.Half, 0.9f), core);
                        }
                        else
                        {
            ...
            if ((flags & DebugDraw.DrawFlags.Pair) != 0)
            {
                ...
                for (int i = 0; i < PairManager.TableCapacity; ++i)
                {
                    ...
                    while (index != PairManager.NullPair)
                    {
                        ...
                        Vec2 x1 = Fix64.Half * (b1.LowerBound + b1.UpperBound);
                        Vec2 x2 = Fix64.Half * (b2.LowerBound + b2.UpperBound);
