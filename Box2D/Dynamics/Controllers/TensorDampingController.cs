...
namespace Box2DX.Dynamics.Controllers
{
    ...
    public class TensorDampingController : Controller
    {
        ...
        public void SetAxisAligned(Fix64 xDamping, Fix64 yDamping)
        {
            ...
            T.Col2.X = Fix64.Zero;
            T.Col2.Y = -yDamping;
            if (xDamping > Fix64.Zero || yDamping > Fix64.Zero)
            {
                MaxTimestep = Fix64.One / Math.Max(xDamping, yDamping);
            }
            else
            {
                MaxTimestep = Fix64.Zero;
            }
        }
        ...
        public override void Step(TimeStep step)
        {
            ...
            if (timestep <= Settings.FLT_EPSILON)
                return;
            if (timestep > MaxTimestep && MaxTimestep > Fix64.Zero)
