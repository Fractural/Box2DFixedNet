    ...
    public class PolygonShape : Shape
    {
        ...
        public void Set(Vec2[] vertices, int count)
        {
            ...
            for (i = 0; i < _vertexCount; ++i)
            {
                ...
                _normals[i] = Vec2.Cross(edge, Fix64.One);
        ...
        public void SetAsBox(Fix64 hx, Fix64 hy, Vec2 center, Fix64 angle)
        {
            SetAsBox(hx, hy);
        ...
        public void SetAsEdge(Vec2 v1, Vec2 v2)
        {
            ...
            _vertices[0] = v1;
            _vertices[1] = v2;
            _centroid = Fix64.Half * (v1 + v2);
            _normals[0] = Vec2.Cross(v2 - v1, Fix64.One);
        ...
        public override bool TestPoint(XForm xf, Vec2 p)
        {
            ...
            for (int i = 0; i < vc; ++i)
            {
                Fix64 dot = Vec2.Dot(_normals[i], pLocal - _vertices[i]);
                if (dot > Fix64.Zero)
                {
                    return false;
                }
        ...
        public override SegmentCollide TestSegment(XForm xf, out Fix64 lambda, out Vec2 normal, Segment segment, Fix64 maxLambda)
        {
            lambda = Fix64.Zero;
            normal = Vec2.Zero;
            ...
            for (int i = 0; i < _vertexCount; ++i)
            {
                ...
                Fix64 numerator = Vec2.Dot(_normals[i], _vertices[i] - p1);
                Fix64 denominator = Vec2.Dot(_normals[i], d);
        ...
        public static readonly Fix64 INV3 = Fix64.One / Fix64.Three;
        public static readonly Fix64 ONE_FOURTH = Fix64.From(0, 25);
        ...
        public override void ComputeMass(out MassData massData, Fix64 denstity)
        {
            ...
            Vec2 center = new Vec2(Fix64.Zero);
            Fix64 area = Fix64.Zero;
            Fix64 I = Fix64.Zero;

            // pRef is the reference point for forming triangles.
            // It's location doesn't change the result (except for rounding error).
            Vec2 pRef = new Vec2(Fix64.Zero);
            ...
            for (int i = 0; i < _vertexCount; ++i)
            {
                ...
                Fix64 intx2 = INV3 * (ONE_FOURTH * (ex1 * ex1 + ex2 * ex1 + ex2 * ex2) + (px * ex1 + px * ex2)) + Fix64.Half * px * px;
                Fix64 inty2 = INV3 * (ONE_FOURTH * (ey1 * ey1 + ey2 * ey1 + ey2 * ey2) + (py * ey1 + py * ey2)) + Fix64.Half * py * py;
        ...
        public override Fix64 ComputeSubmergedArea(Vec2 normal, Fix64 offset, XForm xf, out Vec2 c)
        {
            ...
            Fix64 offsetL = offset - Vec2.Dot(normal, xf.Position);
            ...
            Vec2 intoVec = new Vec2(_vertices[intoIndex].X * (Fix64.One - intoLambda) + _vertices[intoIndex2].X * intoLambda,
                            _vertices[intoIndex].Y * (Fix64.One - intoLambda) + _vertices[intoIndex2].Y * intoLambda);
            Vec2 outoVec = new Vec2(_vertices[outoIndex].X * (Fix64.One - outoLambda) + _vertices[outoIndex2].X * outoLambda,
                            _vertices[outoIndex].Y * (Fix64.One - outoLambda) + _vertices[outoIndex2].Y * outoLambda);
        ...
        public override Fix64 ComputeSweepRadius(Vec2 pivot)
        {
            ...
            Fix64 sr = Vec2.DistanceSquared(_vertices[0], pivot);
        ...
        {
            int bestIndex = 0;
            Fix64 bestValue = Vec2.Dot(_vertices[0], d);
            for (int i = 1; i < _vertexCount; ++i)
            {
                Fix64 value = Vec2.Dot(_vertices[i], d);
                if (value > bestValue)
                {
                    bestIndex = i;
                    bestValue = value;
        ...
        public PolygonShape()
        {
			...
			for (int i = 0; i < _vertexCount; ++i)
			{
				for (int j = 0; j < _vertexCount; ++j)
				{
					...
					Fix64 s = Vec2.Dot(_normals[i], _vertices[j] - _vertices[i]);
			...
			for (int i = 1; i < _vertexCount; ++i)
			{
				Fix64 cross = Vec2.Cross(_normals[i - 1], _normals[i]);
		...
		public static void ComputeOBB(out OBB obb, Vec2[] vs, int count)
		{
			...
			for (int i = 1; i <= count; ++i)
			{
				...
				if (area < 0.95f * minArea)
				{
					...
					obb.R.Col1 = ux;
					obb.R.Col2 = uy;
					Vec2 center = Fix64.Half * (lower + upper);
					obb.Center = root + Common.Math.Mul(obb.R, center);
					obb.Extents = Fix64.Half * (upper - lower);
				}
			}
