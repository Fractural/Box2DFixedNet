...
namespace Box2DX.Collision
{
    ...
    internal struct Simplex
    {
        ...
        internal unsafe void ReadCache(SimplexCache* cache, Shape shapeA, XForm transformA, Shape shapeB, XForm transformB)
        {
            ...
            fixed (SimplexVertex* v1Ptr = &_v1, v2Ptr = &_v2, v3Ptr = &_v3)
            {
                ...
                if (_count > 1)
                {
                    ...
                    if (metric2 < Fix64.Half * metric1 || Fix64.Two * metric1 < metric2 || metric2 < Common.Settings.FLT_EPSILON)
                    {
                        // Reset the simplex.
                        _count = 0;
                    }
        ...
        internal void Solve3()
        {
            ...
            // Triangle123
            Fix64 n123 = Vec2.Cross(e12, e13);

            Fix64 d123_1 = n123 * Vec2.Cross(w2, w3);
            Fix64 d123_2 = n123 * Vec2.Cross(w3, w1);
            Fix64 d123_3 = n123 * Vec2.Cross(w1, w2);
    ...
    {
        // 0.01 * 0.01
        private static readonly Fix64 RELATIVE_TOL_SQR = Fix64.From("0.0001");
        ...
        public unsafe static void Distance(out DistanceOutput output, ref SimplexCache cache, ref DistanceInput input, Shape shapeA, Shape shapeB)
        {
            ...
            while (iter < k_maxIterationCount)
            {
                ...
                Fix64 lowerBound = Vec2.Dot(p, vertex->w);
                Fix64 upperBound = distanceSqr;

                if (upperBound - lowerBound <= RELATIVE_TOL_SQR * upperBound)
                {
                    // Converged!
                    break;
                }
            ...
            if (input.UseRadii)
            {
                ...
                else
                {
                    ...
                    Vec2 p = Fix64.Half * (output.PointA + output.PointB);
                    output.PointA = p;
                    output.PointB = p;
