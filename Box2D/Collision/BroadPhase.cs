        ...
        int QuerySegment(Segment segment, object[] userData, int maxCount, SortKeyFunc sortKey)
        {
            ...
            if (sortKey != null)
            {
                ...
                while (i < _queryResultCount - 1)
                {
                    ...
                    if ((a < Fix64.Zero) ? (b >= Fix64.Zero) : (a > b && b >= Fix64.Zero))
                    {
                ...
                while (_queryResultCount > 0 && _querySortKeys[_queryResultCount - 1] < Fix64.Zero)
                    _queryResultCount--;
            }
            ...
            for (; ; )
            {
                Fix64 xProgress = Fix64.Zero;
                Fix64 yProgress = Fix64.Zero;
                ...
                if (sx != 0)
                {
                    ...
                    xProgress = ((Fix64)_bounds[0][xIndex].Value - p1x) / dx;
                }
                if (sy != 0)
                {
                    //Move on to the next bound
                    ...
                    yProgress = ((Fix64)_bounds[1][yIndex].Value - p1y) / dy;
                }
                for (; ; )
                {
