...
namespace Box2DX.Collision
{
    public partial class Collision
    {
        ...
        public static Fix64 EdgeSeparation(PolygonShape poly1, XForm xf1, int edge1, PolygonShape poly2, XForm xf2)
        {
            ...
            {
                Fix64 dot = Vec2.Dot(vertices2[i], normal1);
                if (dot < minDot)
                {
                    minDot = dot;
                    index = i;
            ...
            Fix64 separation = Vec2.Dot(v2 - v1, normal1World);
            return separation;
        }
        ...
        public static Fix64 FindMaxSeparation(ref int edgeIndex, PolygonShape poly1, XForm xf1, PolygonShape poly2, XForm xf2)
        {
            ...
            {
                Fix64 dot = Vec2.Dot(normals1[i], dLocal1);
                if (dot > maxDot)
                {
                    maxDot = dot;
                    edge = i;
            ...
            PolygonShape poly1, XForm xf1, int edge1, PolygonShape poly2, XForm xf2)
        {
            ...
            for (int i = 0; i < count2; ++i)
            {
                Fix64 dot = Vec2.Dot(normal1, normals2[i]);
                if (dot < minDot)
                {
                    minDot = dot;
                    index = i;
        ...
        private static readonly Fix64 RELATIVE_TOL = Fix64.From("0.98");
        private static readonly Fix64 ABSOLUTE_TOL = Fix64.From("0.001");
            ...
            PolygonShape polyA, XForm xfA, PolygonShape polyB, XForm xfB)
        {
            ...
            if (separationB > RELATIVE_TOL * separationA + ABSOLUTE_TOL)
            {
                poly1 = polyB;
                poly2 = polyA;
                xf1 = xfB;
            ...
            Vec2 localNormal = Vec2.Cross(dv, Fix64.One);
            localNormal.Normalize();
            Vec2 planePoint = Fix64.Half * (v11 + v12);

            Vec2 sideNormal = Common.Math.Mul(xf1.R, v12 - v11);
            sideNormal.Normalize();
            Vec2 frontNormal = Vec2.Cross(sideNormal, Fix64.One);

            v11 = Common.Math.Mul(xf1, v11);
            v12 = Common.Math.Mul(xf1, v12);

            Fix64 frontOffset = Vec2.Dot(frontNormal, v11);
            Fix64 sideOffset1 = -Vec2.Dot(sideNormal, v11);
            Fix64 sideOffset2 = Vec2.Dot(sideNormal, v12);
            ...
            {
                Fix64 separation = Vec2.Dot(frontNormal, clipPoints2[i].V) - frontOffset;
